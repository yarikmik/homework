...
Vlan_num = input('Введите номер влана:')

for line in Cam_conf: # Модуль удаляет пустые строки
	if not (len(line)>0):
		del Cam_conf[Cam_conf.index(line)]

for line in Cam_conf: # Модуль проверяет наличие числа в первом элементе строки
	line = line.split()
	if line[0].isdigit():
...
for line in Cam_print: # Модуль выводит на принт только строки с вланом необходимым пользователю
